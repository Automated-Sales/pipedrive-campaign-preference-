...
try {
  CATEGORY_OPTIONS = JSON.parse(process.env.CATEGORY_OPTIONS || '[]');
  if (!Array.isArray(CATEGORY_OPTIONS) || CATEGORY_OPTIONS.length === 0) {
    throw new Error('CATEGORY_OPTIONS must be a non-empty JSON array.');
  }
} catch (error) {
  throw new Error(`Invalid CATEGORY_OPTIONS: ${error.message}`);
}
...
function parseSetValue(rawValue) {
  ...
  if (typeof rawValue === 'object') {
    if (Array.isArray(rawValue.values)) return rawValue.values.map(String);
...
async function findPersonByToken(token) {
  // Pipedrive person search can search custom fields. Exact-match filtering is also applied locally.
...
function renderPage({ token, person, selectedIds = [], success = false, error = '' }) {
  ...
  const successHtml = success ? `<div class="notice success">Your email preferences have been updated.</div>` : '';
...
<head>
  ...
  <style>
    ...
    .card { background:#fff; border:1px solid #e5e7eb; border-radius:16px; padding:32px; box-shadow:0 8px 30px rgba(0,0,0,.06); }
    h1 { margin:0 0 12px; font-size:30px; line-height:1.15; }
    p { line-height:1.55; color:#4b5563; }
    .option { display:flex; align-items:center; gap:12px; padding:14px 0; border-top:1px solid #eef0f3; font-size:17px; }
    .option input { width:20px; height:20px; }
    .buttons { display:flex; gap:12px; margin-top:24px; align-items:center; }
    button { border:0; border-radius:10px; padding:13px 18px; font-weight:700; cursor:pointer; background:#111827; color:#fff; font-size:16px; }
...
function renderError(message, status = 400) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Error</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f6f7f9;margin:0;color:#1f2937}.card{max-width:620px;margin:60px auto;background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:32px;box-shadow:0 8px 30px rgba(0,0,0,.06)}p{color:#4b5563;line-height:1.55}</style></head><body><section class="card"><h1>We couldn’t open your preferences</h1><p>${escapeHtml(message)}</p></section></body></html>`;
}
...
app.get('/preferences', async (req, res) => {
  try {
    ...
    if (!token) return res.status(400).send(renderError('The preference link is missing a token.'));

    const person = await findPersonByToken(token);
    if (!person) return res.status(404).send(renderError('This preference link is invalid or has expired.'));
...
app.post('/preferences', async (req, res) => {
  try {
    ...
    if (!token) return res.status(400).send(renderError('The preference form is missing a token.'));
    ...
    const updatedPerson = { ...person, [PREFERENCES_FIELD_KEY]: selectedIds.join(',') };
    return res.send(renderPage({ token, person: updatedPerson, selectedIds, success: true }));
  } catch (error) {
    console.error(error);
...
app.listen(PORT, () => {
  console.log(`Preference centre running on http://localhost:${PORT}`);
});
